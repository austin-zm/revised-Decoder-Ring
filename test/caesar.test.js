...
describe(('Caesar Shift'),() => {
  it(('Will return a message that is encoded'), () => {
    const expected = 'wklqnixo';
    const actual = caesar('Thinkful', 3);
    expect(actual).to.eql(expected); 
  });
  it(('Will return a message that is decoded'), () => {
    const expected = 'thinkful';
    const actual = caesar('wklqnixo', 3, false);
    expect(actual).to.eql(expected); 
  });
  it(('If input or shift value is missing, return false '), () => {
    const expected = false;
    const actual = caesar('');
    expect(actual).to.eql(expected); 
  });
  it(('If shift value == 0, return false '), () => {
    const expected = false;
    const actual = caesar('Thinkful', 0);
    expect(actual).to.eql(expected); 
  });
  it(('If shift value = 25, return false'), () => {
    const expected = false;
    const actual = caesar('Thinkful', 30);
    expect(actual).to.eql(expected); 
  });
  it(('Will return false if shift value is equal to 25'), () => {
    const expected = false;
    const actual = caesar('Thinkful', -30);
    expect(actual).to.eql(expected); 
  });
